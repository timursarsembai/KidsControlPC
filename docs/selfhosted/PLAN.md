...
11. ✅ **Отложенное.** Закрыто целиком:
    ...
    - ✅ **Чат.** Сделано 24.08.2026: переписка родителя и детского ПК,
      вложения, отметки о доставке и прочтении. Сообщение доходит до другой
      стороны за 70–110 мс.

      Устройство не получает канал, ключом которого служит аккаунт: он показал
      бы ему переписку остальных детей. Агент подписывается только на те чаты,
      в которых состоит, и список чатов опрашивает — он меняется редко.
